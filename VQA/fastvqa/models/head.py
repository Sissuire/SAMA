...
class VQAHead_samaw(nn.Module):
    ...
    def __init__(
        ...
        self.fc_last = nn.Conv3d(self.hidden_channels, 1, (1, 1, 1))
        self.gelu = nn.GELU()
        self.fc_scale_hid = nn.Conv3d(self.in_channels, self.hidden_channels, (1, 1, 1))
        self.fc_scale_last = nn.Conv3d(self.hidden_channels, 1, (1, 1, 1))
    ...
    def forward(self, x, rois=None):
        ...
        qlt_weight = self.fc_scale_last(self.dropout(self.gelu(self.fc_scale_hid(self.dropout(x_mean)))))
        qlt_weight = self.softmax(qlt_weight)
        return qlt_score * qlt_weight
