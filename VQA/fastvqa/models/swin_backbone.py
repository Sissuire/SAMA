...
global_counter_basiclayer = 0
global_counter_block = 0
global_scale = 16
...
class WindowAttention3D(nn.Module):
    ...
    def __init__(
        ...
        relative_coords = (
            coords_flatten[:, :, None] - coords_flatten[:, None, :]  #  [3, (8*7*7), 1] - [3, 1, (8*7*7)] = [3, 392, 392]
        )  # 3, Wd*Wh*Ww, Wd*Wh*Ww
        ...
        # =========================================================
        # ============================ get pair-wise relative scale index for each token inside the window
        # scale = global_scale
        # scale = self.window_size[0] if scale > self.window_size[0] else scale
        # coords_scale = torch.arange(scale).reshape(-1, 1, 1).repeat(self.window_size[0] // scale, self.window_size[1], self.window_size[2]).flatten(0)
        # relative_scale_index = (coords_scale[:, None] - coords_scale[None, :])  + scale - 1 # 3, Wd*Wh*Ww, Wd*Wh*Ww
        # self.register_buffer("relative_scale_index", relative_scale_index)
        # self.relative_scale_bias_table = nn.Parameter(torch.ones(2 * scale - 1, num_heads))
    ...
    def forward(self, x, mask=None, fmask=None, resized_window_size=None):
        ...
        if resized_window_size is None:
            ...
            # -------------------------------------# 
            # rpi_scale = self.relative_scale_index[:N, :N]   
        else:
            ...
            # -------------------------------------------# 
            # rpi_scale = self.relative_scale_index.reshape(*self.window_size, *self.window_size)[:d,:h,:w,:d,:h,:w]
        ...
        # ---------------------------------------------  
        # relative_scale_bias = self.relative_scale_bias_table[rpi_scale.reshape(-1)].reshape(N, N, -1).permute(2, 0, 1).contiguous()
        ...
        if fmask is not None:
            ...
            ) + relative_position_bias.unsqueeze(0) # + relative_scale_bias.unsqueeze(0).unsqueeze(0)
            attn = attn.view(-1, self.num_heads, N, N)
        else:
            attn = attn + relative_position_bias.unsqueeze(0) # + relative_scale_bias.unsqueeze(0)  # B_, nH, N, N
        # attn = attn + relative_position_bias.unsqueeze(0) + relative_scale_bias.unsqueeze(0)  # B_, nH, N, N
...
class SwinTransformerBlock3D(nn.Module):
    ...
    # def forward_part1(self, x, mask_matrix_swin, mask_matrix_tmp, resized_window_size=(8, 7, 7)): 
    def forward_part1(self, x, mask_matrix, resized_window_size=(8, 7, 7)):
        B, D, H, W, C = x.shape
        ...
        # print("in block layers {}".format(shift_size))
        ...
        if any(i > 0 for i in shift_size):
            ...
            # attn_mask = torch.minimum(mask_matrix_swin, mask_matrix_tmp)  #
        else:
            shifted_x = x
            if False:  # not hasattr(self, 'finfo_windows'):
                shifted_finfo = finfo
            attn_mask = None
            # attn_mask = mask_matrix_tmp  # 
        # partition windows
    ...
    def forward(self, x, mask_matrix, resized_window_size=None):
        ...
        if not self.jump_attention:
            if self.use_checkpoint:
                ...
                # x = checkpoint.checkpoint(self.forward_part1, x, mask_matrix[0], mask_matrix[1])  # 
            else:
                x = self.forward_part1(x, mask_matrix, resized_window_size)
                # x = self.forward_part1(x, mask_matrix[0], mask_matrix[1], resized_window_size)  # 

            # ------------------------------------------------------- scale block MLP
            # ~ Squeeze and Excitation  
            # favg = self.avg_fc(torch.mean(x, dim=(-1, -2, -3), keepdim=True))
            # fmax = self.max_fc(torch.mean(x, dim=(-1, -2, -3), keepdim=True))
            # scale_attn = self.sigmoid(favg + fmax) * 2
            # x = x * scale_attn
...
def compute_mask(D, H, W, window_size, shift_size, device):
    ...
    # ---------------------------- 
    # tmp_attn_mask = None
    # tmp_mask_H = torch.arange(H, device=device).view(1, 1, -1, 1, 1).repeat((1, D, 1, W, 1))
    # tmp_mask_W = torch.arange(W, device=device).view(1, 1, 1, -1, 1).repeat((1, D, H, 1, 1))
    # tmp_mask_H_windows = window_partition(tmp_mask_H, window_size).squeeze(-1)  # nW, ws[0]*ws[1]*ws[2]
    # tmp_mask_W_windows = window_partition(tmp_mask_W, window_size).squeeze(-1)  # nW, ws[0]*ws[1]*ws[2]
    # tmp_attn_mask = torch.abs(tmp_mask_H_windows.unsqueeze(1) - tmp_mask_H_windows.unsqueeze(2)) + torch.abs(tmp_mask_W_windows.unsqueeze(1) - tmp_mask_W_windows.unsqueeze(2))
    # tmp_attn_mask = tmp_attn_mask.masked_fill(tmp_attn_mask > 2, float(-100.0)).masked_fill(tmp_attn_mask <= 2, float(0.0))

    # tmp_mask_s = torch.arange(window_size[0],device=device).view(1, -1).repeat_interleave(window_size[1] * window_size[2]).repeat((tmp_attn_mask.shape[0], 1))
    # tmp_mask_s = tmp_mask_s.unsqueeze(1) - tmp_mask_s.unsqueeze(2)
    # tmp_attn_mask = tmp_attn_mask.masked_fill(tmp_mask_s == 0, float(0.0))  # 当前帧全局空间注意力

    # print(attn_mask.shape)
    # return (attn_mask, tmp_attn_mask)
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, resized_window_size=None):
        ...
        # print("in basic layers {}".format(shift_size))

        x = rearrange(x, "b c d h w -> b d h w c")
        Dp = int(np.ceil(D / window_size[0])) * window_size[0]
        Hp = int(np.ceil(H / window_size[1])) * window_size[1]
        Wp = int(np.ceil(W / window_size[2])) * window_size[2]
        attn_mask = compute_mask(Dp, Hp, Wp, window_size, shift_size, x.device)  # [16, 56, 56] -> 28 -> 14 -> 7
